...
class Model:
    ...
    def set_pepflip_peaks(self):
        peaks = map_peaks_around_molecule_py(
            self.imap_diff, 4.62567528, False, self.imol
        )
...
def imolmapdiff_chooser(callback):
    ...
    imaps = [
        ...
        if valid_map_molecule_qm(i) and not map_is_difference_map(i)
    ]
    imap_diffs = [
        i
        for i in molecule_number_list()
        if valid_map_molecule_qm(i) and map_is_difference_map(i)
    ]
    if len(imols) == 0 or len(imaps) == 0 or len(imap_diffs) == 0:
        print(
            "ML_CORRECTNESS: Can't use ML Correctness: must have a valid model, map and difference map"
        )
        return
