...
class ModelCraft(Pipeline):
    ...
    def run(self):
        ...
        parrot = self.parrot(args.phases)
        buccaneer = self.buccaneer(parrot.abcd, parrot.fphi)
        refmac = self.refmac(buccaneer.structure, cycles=5)
        parrot = self.parrot(refmac.abcd, refmac.fphi_best, refmac.structure)
        buccaneer = self.buccaneer(parrot.abcd, parrot.fphi, refmac.structure)
        refmac = self.refmac(buccaneer.structure, cycles=5)
